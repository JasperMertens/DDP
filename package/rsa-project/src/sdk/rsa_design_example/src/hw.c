...
void hw_mod_exp(uint32_t *msg, uint32_t *exp, uint32_t exp_len, uint32_t *n, uint32_t *R ,uint32_t *R2, uint32_t *res) {
	...
	for(i = 0; i < 16; i++)
	    A[i] = R[i];
	hw_montgomery_multiply_A_B(A, A, 16); // To have A as the last montgomery multiplication result
	...
	while(exp_len>0)
	{
		...
		hw_montgomery_square_prev();
		if(bit)
		{
			// Calculate A = MontMul(A, x_tilde)
			hw_montgomery_multiply_B(x_tilde, 16);
		}
	}

	// Calculate A = MontMul(A, 1)
	//   One is defined in global.h
	hw_montgomery_multiply_B_result((unsigned int *)One, A, 16);
